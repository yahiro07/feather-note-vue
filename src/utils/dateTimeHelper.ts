...
type TimeUnitKey = 'second' | 'minute' | 'hour' | 'day' | 'month' | 'year'

type ElapsedTimeTextSource = {
  prefix: string
...
function formatElapsedTimeText(
  value: number,
  unitKey: TimeUnitKey,
  textSource: ElapsedTimeTextSource
) {
...
const elapsedTimeTextSource_ja: ElapsedTimeTextSource = {
  prefix: '',
  second: '秒',
  minute: '分',
  hour: '時間',
...
export function getElapsedTimeText(timestamp: number, currentTimestamp?: number): string {
  return dateHelpers_getElapsedTimeText_impl(timestamp, elapsedTimeTextSource_ja, currentTimestamp)
}

export function getDateTimeTextWithElapsed(timestamp: number, currentTimestamp?: number): string {
